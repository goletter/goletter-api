...
class FacebookAccount
{
     ...
     * @return array 更新后的账户信息
     */
    public function updateAccountTimezone(string $accountId, int $timezoneId): array
    {
        $businessInfo = [
     ...
     * @return array 更新后的账户信息
     */
    public function updateAccountBusinessInfo(string $accountId): array
    {
        $business_info = [
            'business_country_code' => 'US',
            'business_street' => 'Beach',
     ...
     * 带去重的分页生成器
     */
    protected function paginateWithDeduplication(string $uri, array $query, string $dedupeField = 'id', string $type = ''): \Generator
    {
        $seen = [];
        
        foreach ($this->client->paginate($uri, $query, "GET:/{business_id}/{$type}") as $item) {
